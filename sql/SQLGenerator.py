...
class SQLGenerator(object):
    def __init__(self, tree, schema_graph):
        self.node_dict = {}
        # print "generate_node_dict Start : \n"
        # print tree
        self.generate_node_dict(tree)
        
        # print "generate_node_dict END : \n"
    ...
    def generate_node_dict(self, node):
        ...
        if isinstance(node, AttributeNode):
            ...
            table = values[0]
            # print '[ values: ' , values ,' sql : ',node.sql_label , ' table ' , table , ' attribute ' , node.attribute , ' operator ' , node.operator, ' label ' , node.label , ' ]' 
            self.add_to_node_dict("tables", table)
            self.add_to_node_dict(node.sql_label, "%s %s '%s'" % (node.attribute, node.operator,
                                                                  node.label))
        elif isinstance(node, TableNode):
            # print node.label , "table node"
            # customer which is table 
    ...
    def get_sql(self):
        ...
        try:
            ...
            raise ValueError ,"Can not be mapped "
